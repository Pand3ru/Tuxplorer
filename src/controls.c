...
void ctrlPinView() {
  int sel = 0, start = 0,
      itemAmount = LINES - 4 - 4; /* Calculates number of max. Printed items */
  ...
  while (boolean) {
    ...
    printFolderMenu(leftPanelWindow, sel % amount_pins, start, 2, pins,
                    amount_pins);
    drawTopbars();
  }
  endwin();
  ...
  while (boolean) {
    ...
    case '\n':
      if (dir[sel]->s == F_FALSE) {
        if (isMediaFile(dir[sel]->name)) {
          playMedia(dir[sel]->name);
        } else {
          openInTexteditor(sel);
        }
        return;
      }
      wclear(footerWindow);
    ...
    printFolderMenu(rightPanelWindow, sel % amount_folder, start, 3, dir,
                    amount_folder);
    drawTopbars();
  }
  endwin();
